...
const state = {
  ...
  // 朋友圈图片池
  selectedPoolImage: "",
  // 人物参考图（cloud / local 模式，多选最多3张，保序）
  selectedRefImages: [],
  refTempSelection: [],
};
...
function renderFriendList(keyword = "") {
  ...
  state.friends
    .filter((f) => !keyword || f.name.includes(keyword))
    .forEach((f) => {
      ...
      div.innerHTML = `
        ...
        <div class="friend-info">
          <div class="friend-top">
            ...
            <span class="friend-time">${fmtTime(f.last_time)}</span>
          </div>
          <div class="friend-bottom">
            <span class="last-msg">${esc(f.last_message)}</span>
...
function appendMessage(m, ragContext) {
  ...
  const imageHtml = m.image_url
    ? `<img class="bubble-img" src="${m.image_url}" alt="聊天图片" title="点击查看大图">`
    : "";

  const textHtml = m.content
    ? `<div class="bubble-text">${esc(m.content)}</div>`
    : "";

  let bubbleInner = imageHtml + textHtml;
  if (!bubbleInner) {
    bubbleInner = `<div class="bubble-text">……</div>`;
  }
  ...
  const bodyHtml = `
    <div class="msg-body">
      ...
      ${
        ragContext
          ? `<details class="rag-debug"><summary>本次检索到的角色记忆</summary>${esc(ragContext)}</details>`
          : ""
      }
    </div>
  `;
...
async function sendMessage() {
  ...
  try {
    ...
    if (pendingFile) {
      ...
      if (state.playerName) {
        fd.append("player_name", state.playerName);
      }

      const res = await fetch(`/api/chat/${id}/send-image`, {
        method: "POST",
        body: fd,
      });
      ...
      data = await api(`/api/chat/${id}/send`, {
        method: "POST",
        body: JSON.stringify({
          content,
          player_name: state.playerName || undefined,
        }),
      });
    }
...
function initChatImage() {
  const btn = $("#btn-choose-chat-image");
  const input = $("#chat-image-input");
  const removeBtn = $("#btn-remove-chat-image");
  ...
  input.addEventListener("change", () => {
    ...
    reader.onload = () => {
      ...
      const previewBox = $("#chat-image-preview");
  ...
  if (removeBtn) {
    removeBtn.onclick = clearChatImagePreview;
  }
}
...
function initImageZoom() {
  document.addEventListener("click", (e) => {
    ...
    if (!target) return;
    const zoomable =
      target.classList.contains("bubble-img") ||
      target.classList.contains("moment-img");
    if (!zoomable) return;
    ...
    modal.style.cssText = `
      ...
      z-index: 1000; cursor: zoom-out; opacity: 0; transition: opacity 0.2s;
    `;
    ...
    img.style.cssText = `
      ...
      box-shadow: 0 10px 30px rgba(0,0,0,0.5); transform: scale(0.9); transition: transform 0.2s;
    `;
...
function connectWS() {
  ...
  state.ws.onmessage = (ev) => {
    ...
    if (data.type === "message") {
      ...
      if (state.current === m.character_id && !$("#chat-view").classList.contains("hidden")) {
        appendMessage(m);
        scrollBottom();
      } else {
        state.unread[m.character_id] = (state.unread[m.character_id] || 0) + 1;
        saveUnread();
      }
      ...
      if (!$("#moments-panel").classList.contains("hidden")) {
        loadMoments();
      }
    }
  };
...
async function loadMoments() {
  ...
  posts.forEach((p) => {
    ...
    el.innerHTML = `
      <div class="moment-head">
        ...
        <div>
          <div class="moment-name">${esc(p.character_name)}</div>
          <div class="moment-loc">📍 ${esc(p.location || "")} · ${esc(p.created_at.slice(5, 16))}</div>
        </div>
      </div>
      ...
      <div class="moment-actions">
        <button class="like-btn ${state.liked.includes(p.id) ? "liked" : ""}" data-id="${p.id}">
          💗 ${p.likes}
        </button>
      </div>
      ...
      <div class="comment-input">
        <input placeholder="评论…" data-id="${p.id}">
        <button class="comment-btn" data-id="${p.id}">发送</button>
      </div>
    `;
  ...
  feed.querySelectorAll(".like-btn").forEach((btn) => {
    btn.onclick = async () => {
      ...
      if (state.liked.includes(id)) return;
  ...
  feed.querySelectorAll(".comment-btn").forEach((btn) => {
    btn.onclick = async () => {
      ...
      await api(`/api/moments/${id}/comment`, {
        method: "POST",
        body: JSON.stringify({ content, author_name: state.playerName || undefined }),
      });
...
function renderMyAvatarPreview() {
  const box = $("#my-avatar-preview");
  if (!box) return;

  if (state.playerAvatar) {
    box.style.backgroundImage = `url("${state.playerAvatar}")`;
    box.textContent = "";
  } else {
    box.style.backgroundImage = "";
    box.textContent = (state.playerName || "我").slice(0, 1);
  }
}
...
async function loadProfile() {
  try {
    ...
    if (state.playerAvatar) {
      localStorage.setItem("player_avatar", state.playerAvatar);
    } else {
...
async function openSettings() {
  ...
  $("#status-box").innerHTML = `
    <div>API Key：${s.api_key_configured ? "✅ 已配置" : "❌ 未配置（请在 .env 填写）"}</div>
    <div>模型：${esc(s.model)}　|　接口：${esc(s.base_url)}</div>
    <div>图像生成：${s.image_api_configured ? "✅ 已启用" : "⬜ 未配置（使用内置占位图）"}</div>
    <div>角色数量：${s.character_count}　|　RAG 知识片段：${s.rag_chunks} 条</div>
  `;
}

// ============ 朋友圈生成弹窗 ============
function populateGenCharacterSelect() {
  const select = $("#gen-character");
  if (!select) return;
  ...
  // 保持之前选中的角色；若不存在则默认选中第一个角色
  if (current && state.friends.some((f) => f.id === current)) {
    select.value = current;
  } else if (state.friends.length > 0) {
    select.value = state.friends[0].id;
  }
}
...
function openGenMomentModal() {
  ...
  $("#gen-prompt").value = "";
  $("#gen-caption-mode").value = "vision";
  $("#gen-people").value = "";
  $("#gen-manual-caption").value = "";
  clearPoolImage();
...
function closeGenMomentModal() {
  $("#gen-moment-modal").classList.add("hidden");
}
...
async function confirmGenMoment() {
  ...
  if (source === "pool") {
    ...
    if ((source === "cloud" || source === "local") && state.selectedRefImages.length) {
      payload.reference_images = state.selectedRefImages;
    }
  }
  ...
  try {
    if (source === "pool") {
      const cm = $("#gen-caption-mode").value;
      setGenStatus(cm === "vision" ? "正在让 AI 识别图片并生成文案…" : "正在发布动态…");
    } else if (source === "cloud") {
      setGenStatus(state.selectedRefImages.length
        ? "正在调用云端 AI 生图（含人物参考图），可能需要较长时间…"
        : "正在调用云端 AI 生图，可能需要较长时间…");
    } else {
      setGenStatus(state.selectedRefImages.length
        ? "正在调用本地 ComfyUI 生图（含人物参考图），本地生图较慢，请耐心等待…"
        : "正在调用本地 ComfyUI / SD 生图，可能需要较长时间…");
    }

    await api("/api/moments/generate", {
      method: "POST",
    ...
    setGenStatus("生成失败：" + e.message, true);
    toast("❌ 生成失败：" + e.message);
  } finally {
    btn.disabled = false;
    btn.textContent = "生成动态";
  }
}

// ============ 图片池选择器 ============
async function openPoolPicker() {
  $("#pool-picker-modal").classList.remove("hidden");
  await loadPoolImages();
}

function closePoolPicker() {
  $("#pool-picker-modal").classList.add("hidden");
}

async function loadPoolImages() {
  const grid = $("#pool-grid");
  grid.innerHTML = `<div class="pool-loading">加载中…</div>`;

  try {
    const data = await api("/api/moments/pool");
    grid.innerHTML = "";

    if (!data.images || data.images.length === 0) {
      grid.innerHTML = `<div class="pool-empty">图片池为空，请先上传图片</div>`;
      return;
    }
    ...
    grid.innerHTML = `<div class="pool-empty">加载失败：${esc(e.message)}</div>`;
  }
}
...
function selectPoolImage(url) {
  ...
  const preview = $("#gen-pool-preview");
  preview.src = url;
...
function clearPoolImage() {
  ...
  const clearBtn = $("#btn-clear-pool-image");
  if (clearBtn) clearBtn.classList.add("hidden");
}

// ============ 人物参考图选择器（多选，最多3张，保序） ============
async function openRefPicker() {
  state.refTempSelection = [...state.selectedRefImages];
...
async function loadRefImages() {
  ...
  try {
    const data = await api("/api/moments/ref-images");
    grid.innerHTML = "";

    if (!data.images || data.images.length === 0) {
      grid.innerHTML = `<div class="pool-empty">参考图池为空</div>`;
      return;
    }
    ...
    data.images.forEach((img) => {
      ...
      item.dataset.url = img.url;
      item.innerHTML = `<img src="${img.url}" alt="${esc(img.name)}">` +
        (idx >= 0 ? `<span class="ref-order">${idx + 1}</span>` : "");
      item.onclick = () => {
        toggleRefSelect(img.url);
      };
      grid.appendChild(item);
    });
  } catch (e) {
...
function toggleRefSelect(url) {
  ...
  if (idx >= 0) {
    state.refTempSelection.splice(idx, 1);
  } else {
    if (state.refTempSelection.length >= 3) {
      toast("最多选择 3 张参考图");
      return;
    }
    state.refTempSelection.push(url);
  }
  loadRefImages();
}

function confirmRefSelection() {
  state.selectedRefImages = [...state.refTempSelection];
  renderRefPreview();
  closeRefPicker();
}
...
document.querySelectorAll(".nav-btn[data-view]").forEach((btn) => {
  btn.onclick = () => {
    ...
    const view = btn.dataset.view; // chat / moments / mini
...
$("#btn-save-nickname").onclick = async () => {
  ...
  await api("/api/system/profile", {
    method: "PUT",
    body: JSON.stringify({ name: state.playerName }),
  });

  renderMyAvatarPreview();
  toast("昵称已保存，角色们会这样称呼你～");
};

$("#btn-test-greeting").onclick = async () => {
  await api("/api/system/test/greeting", { method: "POST" });
  toast("已触发早安推送，请查看消息列表");
};

$("#btn-choose-avatar").onclick = () => $("#input-avatar").click();

$("#input-avatar").addEventListener("change", async () => {
  const file = $("#input-avatar").files[0];
  if (!file) return;
  ...
  try {
    ...
    state.playerAvatar = data.avatar_url;
    localStorage.setItem("player_avatar", state.playerAvatar);
...
$("#gen-moment-modal").addEventListener("click", (e) => {
  if (e.target === $("#gen-moment-modal")) {
    closeGenMomentModal();
  }
});
...
$("#pool-picker-modal").addEventListener("click", (e) => {
  if (e.target === $("#pool-picker-modal")) {
    closePoolPicker();
  }
});

$("#input-pool-upload").addEventListener("change", async () => {
  const file = $("#input-pool-upload").files[0];
  if (!file) return;

  if (!file.type.startsWith("image/")) {
    toast("请选择图片文件");
    $("#input-pool-upload").value = "";
    return;
  }
...
$("#ref-picker-modal").addEventListener("click", (e) => {
  if (e.target === $("#ref-picker-modal")) {
    closeRefPicker();
  }
});
